import { Users, Loader2, Info, AlertTriangle, CheckCircle, MapPin, Clock } from "lucide-react";
...
import { AccordionItem } from "../guia/_components/Accordion";
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-zinc-900 p-6 rounded-lg border-t-4 border-yellow-500">
              ...
              <p className="text-sm text-gray-400">Kimonos/Roupas limpas, unhas cortadas e pés limpos são obrigatórios para a segurança de todos.</p>
            </div>
            <div className="bg-zinc-900 p-6 rounded-lg border-t-4 border-green-500">
              ...
              <p className="text-sm text-gray-400">Respeite os graduados, o mestre e seus parceiros e parceiras de treino. O ego fica fora do tatame.</p>
            </div>
          </div>

          <div className="lg:col-span-8">
            <h2 id="perguntas-frequentes" className="scroll-mt-24 text-3xl font-display text-primary mb-8 border-b border-zinc-800 pb-2">Perguntas Frequentes</h2>

            <div className="space-y-2">
              <AccordionItem title="Como faço para treinar com vocês? Preciso me inscrever?">
                <p>Nossa equipe é aberta para todos! Não precisa se inscrever ou ter experiência, para treinar com a gente basta ter interesse na modalidade e vir em um dos nossos treinos, onde estaremos sempre de braços abertos para recebê-los!</p>
              </AccordionItem>

              <AccordionItem title="Preciso ter um Kimono?">
                <p>Se você não tiver um kimono não tem problema. Pode ir com uma roupa confortável de fazer exercício físico, de preferência sem bolsos ou zíperes.</p>
              </AccordionItem>

              <AccordionItem title="É cobrado mensalidade?">
                <p>Sim, a mensalidade tem o valor de R$ 70,00. O valor é utilizado para a remuneração dos professores, DM's da modalidade, aquisição de equipamentos, manutenção do espaço, promoção de eventos, seminários e outros.</p>
              </AccordionItem>

              <AccordionItem title="O que eu preciso levar para a primeira aula?">
                <p>Para sua aula experimental, venha com uma roupa confortável e resistente (camiseta e bermuda/calça de ginástica de preferência sem zíperes ou botões). Traga também uma garrafa de água.</p>
              </AccordionItem>

              <AccordionItem title="Como funcionam as graduações?">
                <p>A graduação no Jiu-Jitsu é baseada no tempo de prática, evolução técnica e comportamento. As faixas seguem a ordem: Branca, Azul, Roxa, Marrom e Preta. Não tenha pressa, aproveite a jornada. Temos graduações semestrais, que são alinhadas com os professores. Os graus podem ser dados em qualquer momento ao longo do ano.</p>
              </AccordionItem>
              ...
              <AccordionItem title="Sou aluno de outro curso/instituto, posso participar?">
                <p>O projeto é focado na comunidade da Unicamp (alunos, funcionários e docentes). Todos são bem-vindos a participar!</p>
              </AccordionItem>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {(() => {
                ...
                const horarios = Array.from(horariosSet).sort(
                  (a, b) => getHorarioInicioEmMinutos(a as string) - getHorarioInicioEmMinutos(b as string),
                ) as string[];
                ...
                return horarios.map((horario: string) => (
                  <tr key={horario}>
                    ...
                    {diasSemana.map((dia) => {
                      ...
                      return (
                        <td key={`${dia}-${horario}`} className="border border-zinc-800 p-3 text-center bg-zinc-900/30">
                          {tipo && (
                            <div className="flex flex-col items-center justify-center gap-1">
                              <span className={`text-base font-semibold ${getCorTipo(tipo)}`}>
                                {tipo === "Geral" ? "GERAL" : tipo}
                              </span>
                              <span className="text-xs text-gray-400 leading-none">({professor})</span>
                            </div>
                          )}
                        </td>
                      );
